...
import { createQuiz, updateQuiz, getNextPublishDate } from '../../../api/quizzes';
import type { CreateQuizRequestDto } from 'pai-shared-types';
...
export function QuizCreationModal({
  ...
  const [hint, setHint] = useState(editQuiz?.hint || '');
  const [reward, setReward] = useState(editQuiz?.reward || '');
  ...
  // 다음 출제 가능 날짜 조회
  const { data: nextPublishDateData } = useQuery({
    queryKey: ['next-publish-date'],
    queryFn: getNextPublishDate,
    enabled: !editQuiz, // 수정 모드가 아닐 때만 조회
  });
  ...
  useEffect(() => {
    ...
    } else if (nextPublishDateData) {
      // API 함수가 문자열을 반환하도록 수정했지만, 캐시 때문에 객체가 올 수 있음
      const dateString = typeof nextPublishDateData === 'string'
        ? nextPublishDateData
        : (nextPublishDateData as any).defaultPublishDate;
      setPublishDate(dateString);
    }
  }, [editQuiz, nextPublishDateData]);

  // 퀴즈 생성 mutation
  const createMutation = useMutation({
    mutationFn: (data: CreateQuizRequestDto) => createQuiz(data),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['parent-quizzes'] });
      handleClose();
    },
    onError: (error) => {
  ...
  const updateMutation = useMutation({
    mutationFn: (data: CreateQuizRequestDto) =>
      updateQuiz(editQuiz!.id, {
